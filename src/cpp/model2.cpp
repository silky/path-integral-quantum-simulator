...
amp_t evaluate(workunit_t wu, gate_t gate) {
  ...
  for (int i = 0; i < n_preds; i++) {
    ...
    case H:
      resamp +=
          HapplySlice(bitextract(wu.predecessors[i], gate.qubits),
                      wu.amplitudes[i], bitextract(wu.target, gate.qubits));
      break;
    case CNOT:
      resamp +=
          CNOTapplySlice(bitextract(wu.predecessors[i], gate.qubits),
                         wu.amplitudes[i], bitextract(wu.target, gate.qubits));
      break;
    default:
...
void print_worklist(vector<workunit_t> worklist) {
  ...
  for (workunit_t wu : worklist) {
    cout << "{ .target=" << wu.target << ", .deps=can_eval " << can_eval(wu)
         << " init " << non_default(wu) << ", .depth=" << wu.depth
         << ", .dest=" << ((wu.returnloc == LOCAL) ? "LOCAL " : "REMOTE ")
         << wu.wu_dest << ", idx=" << wu.wu_dst_pred_idx
         << ", .preds=" << print_priors(wu) << "}" << endl;
  }
}
...
struct PrintWorkLists : sc_module {
  ...
  PrintWorkLists(::sc_core::sc_module_name, vector<workunit_t> *worklists_[],
                 int len_worklists_) {
  ...
  void thread_process() {
    ...
    if (1) {
      while (1) {
        ...
        // if (iter % 100 == 0) {
        //     for (int i = 0; i < len_worklists; i++) {
        //         cout << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b" << flush;
        //     }
        // 
        //     for (int i = 0; i < len_worklists; i++) {
        //       cout << worklists[i]->size() << ", ";
        //     }
        //     cout << flush;
        // }
        //cout << "----iteration " << iter << " end\n\n" << endl;
        iter++;
      }
    }
  }
...
struct BaseCase : sc_module {
  ...
  tlm_utils::simple_target_socket<BaseCase, sizeof(workunit_t)>
      workrecv; // just send the wu
  tlm_utils::simple_initiator_socket<BaseCase, sizeof(workreturn_t)>
  ...
  virtual tlm::tlm_sync_enum ProcessBaseCase(tlm::tlm_generic_payload &trans,
    ...
    amp_t amplitude = (wu_ptr->target == wu_ptr->inital) ? (amp_t){1.0, 0.0}
                                                         : (amp_t){0.0, 0.0};
    ...
    workreturn_t *wret =
        new (workreturn_t){.target = wu_ptr->target,
                           .amplitude = amplitude,
...
struct FindAmp : sc_module {
  ...
  tlm_utils::simple_target_socket<FindAmp, sizeof(workunit_t)>
      workrecv; // just send the wu
  tlm_utils::simple_initiator_socket<FindAmp, sizeof(workreturn_t)>
      workret; // return values
  tlm::tlm_generic_payload workret_trans;
  ...
  FindAmp(::sc_core::sc_module_name, int mindepth_)
  ...
  virtual tlm::tlm_sync_enum recv_reply(tlm::tlm_generic_payload &trans,
    ...
    if (DEBUG)
      cout << "got a reply - [" << wret_ptr->wu_dest << "]["
           << wret_ptr->wu_dst_pred_idx << "] = " << wret_ptr->amplitude
           << endl;
    workreplies_queue.push(*wret_ptr);

    // worklist[wret_ptr->wu_dest].amplitudes[wret_ptr->wu_dst_pred_idx] =
    // wret_ptr->amplitude;
  ...
  void CalcAmpSC() {
    ...
    tlm::tlm_phase phase;
    sc_time delay;
    sc_time localdelay = sc_time(0, SC_NS);

    while (1) { // wait for stuff to come to us!
      // worklist_sizes.push_back( (pair<sc_time, int>){sc_time_stamp(),
      // worklist.size()} );
      localdelay += sc_time(10, SC_NS);
      if (localdelay > sc_time(0, SC_NS)) {
        wait(localdelay);
      ...
      if (!workreplies_queue.empty()) {
        ...
        worklist[retval.wu_dest].amplitudes[retval.wu_dst_pred_idx] =
            retval.amplitude;
      ...
      if (wu.depth <= mindepth) {
        ...
        if (DEBUG)
          cout << "reached base state for this block at lvl " << mindepth
               << endl;
        ...
        workrequestor_trans.set_data_ptr(
            reinterpret_cast<unsigned char *>(&workunit_req));
        ...
        if (wu.returnloc == REMOTE) { // this is the target state we were invoked for
          ...
          workreturn_t *wret = new (workreturn_t){
              ...
              .wu_dest = wu.wu_dest,
              .wu_dst_pred_idx =
                  wu.wu_dst_pred_idx // position in the predecessors list
          };
        ...
        for (int i = 0; i < predecessors.size(); i++) {
          worklist.push_back((workunit_t){
              ...
              .predecessors = {}, // zero
              .amplitudes = {},
              // as we only pop from the pack, we can identify the requesting
              // state by its current index.
              .returnloc = LOCAL,
              .wu_dest = dest_wu_idx,
              .wu_dst_pred_idx = i,
          });
...
// split the circuit in half for performance reasons! this is where parallelism
// comes in
struct HeightDiv : sc_module {
  // up-circuit connection
  tlm_utils::simple_target_socket<HeightDiv, sizeof(workunit_t)>
      up_workrequestor;
  tlm_utils::simple_initiator_socket<HeightDiv, sizeof(workreturn_t)>
      up_workreply;
  tlm::tlm_generic_payload workreply_trans;

  // down-circuit connection 1
  tlm_utils::simple_initiator_socket<HeightDiv, sizeof(workunit_t)>
      down_0_workrequestor;
  tlm_utils::simple_target_socket<HeightDiv, sizeof(workreturn_t)>
      down_0_workreply;
  tlm::tlm_generic_payload workrequestor_0_trans;

  // down-circuit connection 2
  tlm_utils::simple_initiator_socket<HeightDiv, sizeof(workunit_t)>
      down_1_workrequestor;
  tlm_utils::simple_target_socket<HeightDiv, sizeof(workreturn_t)>
      down_1_workreply;
  ...
  typedef HeightDiv SC_CURRENT_USER_MODULE;
  HeightDiv(::sc_core::sc_module_name, int split)
      : up_workrequestor("up_workrequestor"), up_workreply("up_workreply"),
        down_0_workrequestor("down_0_workrequestor"),
        down_0_workreply("down_0_workreply"),
        down_1_workrequestor("down_1_workrequestor"),
        down_1_workreply("down_1_workreply"), splitpt(split) {
    up_workrequestor.register_nb_transport_fw(this,
                                              &HeightDiv::WorkRequest_route);
    down_0_workreply.register_nb_transport_fw(this,
                                              &HeightDiv::WorkReply_route);
    down_1_workreply.register_nb_transport_fw(this,
                                              &HeightDiv::WorkReply_route);
  }
  ...
  virtual tlm::tlm_sync_enum WorkRequest_route(tlm::tlm_generic_payload &trans,
    ...
    if (wu.target.to_ulong() > splitpt) { // upper
      workrequestor_0_trans.set_data_ptr(
          reinterpret_cast<unsigned char *>(&wu));
      phase = tlm::BEGIN_REQ;
      down_0_workrequestor->nb_transport_fw(workrequestor_0_trans, phase,
                                            delay);
      return tlm::TLM_ACCEPTED;
    } else {
      workrequestor_1_trans.set_data_ptr(
          reinterpret_cast<unsigned char *>(&wu));
      phase = tlm::BEGIN_REQ;
      down_1_workrequestor->nb_transport_fw(workrequestor_1_trans, phase,
                                            delay);
      return tlm::TLM_ACCEPTED;
    }
  }
...
// if there is a height split, we need to maintain that split for all
// sub-things. perhaps be more specific? explicit tree. basic unit: (findamp,
...
SC_MODULE(Top) {
  ...
  int heightsplits[MAX_MODULES];
  amp_connspec
      ampconns[MAX_MODULES]; // = { {HEIGHTDIV, 0}, {BASE, 0}, {BASE, 1} };
  ...
  moduletypes parsemod(string s) {
    if (s == "INITIATOR ") {
      ...
      cout << "could not parse module type spec \"" << s
           << "\" (mabye there is whitespace?)" << endl;
      exit(5);
    }
  }

  void parse_spec(istream & config) {
    // ifstream config(fname, ios::in);
    string line;
    getline(config, line);
    ...
    for (int i = 0; i < n_divs; i++) {
      std::getline(config, line, ','); // get the split point
      heightsplits[i] = std::stoi(line);
      std::getline(config, line, ','); // get connection type
      modtype = parsemod(line);
      std::getline(config, line, ','); // get conn idx.
  ...
  Top(::sc_core::sc_module_name, istream & config) {
    ...
    for (int i = 0; i < n_divs; i++) {
      ostringstream name;
      name << "HeightDiv:" << i;
      divs[i] = new HeightDiv(name.str().c_str(), heightsplits[i]);
    }
    for (int i = 0; i < n_base_cases; i++) {
      ostringstream name;
      name << "BaseCase:" << i;
      basecases[i] = new BaseCase(name.str().c_str());
    }
    ...
    // by only making requests to the top ampfinder, if we request lower than
    // it's cuttoff, it will process the first workunit whilst distributing the
    // prev levels to lower. this is slightly dumb but not a problem currently,
    // just a counterintuative layering violation. we could fix this in findamp
    // by checking the level before enqueue, but this is probably a net loss.
    // alt fix here by requesting to the right module, but this complicates
    // networking needing 1 more port per findamp for no benifit x2.
    init->socket.bind(amplitude_finders[0]->workrecv);
    init->work_return.bind(amplitude_finders[0]->workret);

    amp_connspec ampconn;
    // in Python I would solve this with ducktyping - not sure how to do the
    // dynamic dispatch on a object of unknown type in cpp without the switch?
    for (int i = 0; i < n_amp_finders; i++) {
      ampconn = ampconns[i];
      switch (ampconn.type) {
      case FINDAMP: // direct connection to the next layer down
        cout << "ampfind direct connection - you might want to merge this!"
             << endl;
        amplitude_finders[i]->workrequestor.bind(
            amplitude_finders[ampconn.idx]->workrecv);
        amplitude_finders[i]->workreply.bind(
            amplitude_finders[ampconn.idx]->workret);
        break;
      case HEIGHTDIV:
        amplitude_finders[i]->workrequestor.bind(
            divs[ampconn.idx]->up_workrequestor);
        amplitude_finders[i]->workreply.bind(divs[ampconn.idx]->up_workreply);
        break;
      case BASE:
        amplitude_finders[i]->workrequestor.bind(
            basecases[ampconn.idx]->workrecv);
        amplitude_finders[i]->workreply.bind(basecases[ampconn.idx]->workret);
        break;
      case INITIATOR:
        cout << "CONNECTION ERROR: tried to make a request to an initator."
             << endl;
        break;
      }
    }
    ...
    for (int i = 0; i < n_divs; i++) {
      ...
      switch (divconn.type) {
      case FINDAMP: // direct connection to the next layer down
        divs[i]->down_0_workrequestor.bind(
            amplitude_finders[divconn.idx[0]]->workrecv);
        divs[i]->down_0_workreply.bind(
            amplitude_finders[divconn.idx[0]]->workret);

        divs[i]->down_1_workrequestor.bind(
            amplitude_finders[divconn.idx[1]]->workrecv);
        divs[i]->down_1_workreply.bind(
            amplitude_finders[divconn.idx[1]]->workret);
        break;
      case HEIGHTDIV:
        cout << "nested divs - you probably don't want this." << endl;
        divs[i]->down_0_workrequestor.bind(
            divs[divconn.idx[0]]->up_workrequestor);
        divs[i]->down_0_workreply.bind(divs[divconn.idx[0]]->up_workreply);

        divs[i]->down_1_workrequestor.bind(
            divs[divconn.idx[1]]->up_workrequestor);
      ...
      case BASE:
        ...
        break;
      case INITIATOR:
        cout << "CONNECTION ERROR: tried to make a request to an initator. you "
                "def dont want this."
             << endl;
        break;
      }
